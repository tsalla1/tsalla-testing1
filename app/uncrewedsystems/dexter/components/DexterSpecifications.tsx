...
const PayloadAccordionItem = ({
  ...
  >
    <div className="flex justify-between items-center">
      ...
      {isOpen ? (
        <Minus className="w-5 h-5 transition-transform text-black" />
      ) : (
...
export default function PayloadCategories() {
  ...
  <div className="flex border border-black w-fit mx-auto">
    {Object.keys(categories).map((key) => (
      <button
        key={key}
        onClick={() => setActive(key as keyof typeof categories)}
        className={`px-6 py-3 text-sm md:text-base font-medium border-r border-black last:border-r-0 transition-all
          ${active === key ? "bg-black text-white scale-115" : "bg-white text-black hover:bg-gray-100 "}
        `}
      >
        ...
        {/* Content */}
        <div className=" justify-center mt-12 grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
